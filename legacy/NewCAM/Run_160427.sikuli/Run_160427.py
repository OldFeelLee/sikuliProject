...
def ConfToolStart():    #ip 복사 및 ConfTool 실행
    ...
    App.open("notepad")
    wait(1)
    type("10.0.127.215")    # ip 입력부분 카메라별로 수정 필요
    keyDown(Key.CTRL+"a")
    wait(1)
    keyUp(Key.CTRL+"a")
    keyDown(Key.CTRL+"c")
...
"""
popup(unicode("확인을 누르면 test가 시작되며,\npopup 내용을 주의깊게 확인해주세요.","utf-8"),"Start")
ConfToolStart()
...
# REC
popup(unicode("storage 설정 및 multistream 녹화 설정, 녹화 스케쥴 설정을 시작합니다.","utf-8"),"REC Schedule Setup")
ConfToolEnter()
